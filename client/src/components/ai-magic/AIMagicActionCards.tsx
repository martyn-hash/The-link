...
import { TasksRemindersModal, OverdueWorkModal } from './TasksRemindersModal';
...
export function ShowOverdueWorkActionCard({ functionCall, onComplete, onDismiss }: ActionCardProps) {
  ...
  const handleClose = () => {
    setIsModalOpen(false);
    onComplete(true, 'Viewed overdue work');
  };
  ...
  if (userNotFound) {
    return (
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        className="bg-gradient-to-br from-red-50 to-orange-50 dark:from-red-950/30 dark:to-orange-950/30 border border-red-200 dark:border-red-800 rounded-xl p-4"
        data-testid="action-card-overdue-work-error"
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-red-700 dark:text-red-400">
            <AlertCircle className="w-4 h-4" />
            <span className="font-medium text-sm">Could not find a team member named "{userName}"</span>
          </div>
          <Button
            size="sm"
            variant="ghost"
  ...
  return (
    <>
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        className="bg-gradient-to-br from-red-50 to-orange-50 dark:from-red-950/30 dark:to-orange-950/30 border border-red-200 dark:border-red-800 rounded-xl p-3 flex items-center gap-3"
      ...
      >
        <div className="flex items-center gap-2 text-red-700 dark:text-red-400">
          ...
          <span className="font-medium text-sm">
            {isModalOpen ? 'Overdue Work' : 'Loading overdue items...'}
          </span>
        </div>
        ...
        {resolvedUserName && (
          <div className="text-sm text-muted-foreground flex items-center gap-1.5">
            <User className="w-3.5 h-3.5" />
            Showing {resolvedUserName}'s overdue work
          </div>
        )}
      </motion.div>
