...
import { CLIENT_FIELD_DEFINITIONS } from "@shared/importTypes";
...
export default function ClientsImport() {
  ...
  const [enrichWithCompaniesHouse, setEnrichWithCompaniesHouse] = useState(true);
  ...
  const handleFileUpload = async () => {
    ...
    try {
      const response = await fetch('/api/clients-import/parse', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
  ...
  const handleValidate = async () => {
    ...
    try {
      const response = await fetch('/api/clients-import/validate', {
        method: 'POST',
  ...
  const executeImport = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/clients-import/execute', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          data: parsedRows,
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto p-6 max-w-7xl">
        <div className="mb-6">
          <h1 className="text-3xl font-bold" data-testid="page-title">Clients Import</h1>
          <p className="text-muted-foreground mt-2">
            Import clients with flexible column mapping and Companies House enrichment
          </p>
        </div>
        ...
        {currentStep === 'upload' && (
          <div className="space-y-6">
            <Alert>
              ...
              <AlertTitle>Clients Import</AlertTitle>
              <AlertDescription>
                <p className="mb-2">Import client companies with automatic data enrichment.</p>
                <ul className="list-disc list-inside space-y-1 text-sm">
                  <li><strong>Duplicate Detection:</strong> Matched by Company Number or Client Name</li>
                  <li><strong>Companies House:</strong> Optionally enrich with official company data</li>
                  <li><strong>Phone Formatting:</strong> UK phone numbers automatically formatted</li>
                  <li><strong>Address Parsing:</strong> Multi-line addresses split into components</li>
                </ul>
              </AlertDescription>
            </Alert>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Select a CSV or Excel file (.csv, .xlsx, .xls) containing your client data
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center space-x-2 p-4 border rounded-lg bg-muted/50">
                  <Switch
                    id="enrich-ch"
                    checked={enrichWithCompaniesHouse}
                    onCheckedChange={setEnrichWithCompaniesHouse}
                    data-testid="switch-enrich-ch"
                  />
                  <Label htmlFor="enrich-ch" className="flex-1">
                    <div className="font-medium">Enrich with Companies House data</div>
                    <div className="text-sm text-muted-foreground">
                      Automatically fetch company status, accounts dates, and official details
                    </div>
                  </Label>
                </div>
              </CardContent>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <Card className="bg-green-50 dark:bg-green-950/20 border-green-200">
                <CardContent className="pt-4 text-center">
                  <div className="text-3xl font-bold text-green-600">{validationResult.matchStats.clientsToCreate}</div>
                  <div className="text-xs text-muted-foreground">To Create</div>
                </CardContent>
              </Card>
              <Card className="bg-blue-50 dark:bg-blue-950/20 border-blue-200">
                <CardContent className="pt-4 text-center">
                  <div className="text-3xl font-bold text-blue-600">{validationResult.matchStats.clientsToUpdate}</div>
                ...
                <Tabs defaultValue="all">
                  <TabsList>
                    ...
                    <TabsTrigger value="create" data-testid="tab-create">
                      To Create ({validationResult.matchStats.clientsToCreate})
                    </TabsTrigger>
                    <TabsTrigger value="update" data-testid="tab-update">
                      To Update ({validationResult.matchStats.clientsToUpdate})
                    </TabsTrigger>
                  </TabsList>
                            ...
                            <TableRow key={item.row} data-testid={`preview-row-${item.row}`}>
                              ...
                              <TableCell>
                                <div className="flex items-center gap-2">
                                  ...
                                  {item.mappedData?.name || item.existingClient?.name || '-'}
                                </div>
                              </TableCell>
                              <TableCell className="font-mono">
                                {item.mappedData?.companyNumber || '-'}
                              </TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                        ...
                        <TableBody>
                          {validationResult.previewData.filter(p => p.action === 'create').map((item) => (
                            <TableRow key={item.row}>
                              <TableCell className="font-mono">{item.row}</TableCell>
                              <TableCell>{item.mappedData?.name || '-'}</TableCell>
                              <TableCell className="font-mono">{item.mappedData?.companyNumber || '-'}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
                    ...
                    <ScrollArea className="h-[300px]">
                      <Table>
                        <TableHeader>
                          <TableRow>
                            <TableHead>Row</TableHead>
                            <TableHead>Existing Client</TableHead>
                            <TableHead>Company Number</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {validationResult.previewData.filter(p => p.action === 'update').map((item) => (
                            <TableRow key={item.row}>
                              <TableCell className="font-mono">{item.row}</TableCell>
                              <TableCell>{item.existingClient?.name || '-'}</TableCell>
                              <TableCell className="font-mono">{item.mappedData?.companyNumber || '-'}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="text-center space-y-4">
                ...
                <h3 className="text-lg font-semibold">Importing Clients...</h3>
                <Progress value={importProgress} className="w-full" />
                <p className="text-sm text-muted-foreground">
                  {enrichWithCompaniesHouse ? 'Fetching Companies House data and importing...' : 'Processing import...'}
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-6">
            <Alert className="bg-green-50 dark:bg-green-950/20 border-green-200">
              ...
              <AlertDescription className="text-green-700 dark:text-green-300">
                Successfully processed {executionResult.summary.created + executionResult.summary.updated} clients.
              </AlertDescription>
            </Alert>
