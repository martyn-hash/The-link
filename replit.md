...
-   **Document E-Signature System**: UK eIDAS-compliant electronic signature feature enabling staff to send PDFs for signature via secure public links. Supports multiple signers per document with sequential or parallel workflows. **Multi-page PDF support** with full page navigation during field placement and signing. Features include: interactive PDF viewer (react-pdf) with page-by-page navigation for signature field placement, precise percentage-based coordinate system (2 decimal places) ensuring accurate field positioning across all PDF sizes/devices, signature and typed name fields with visual indicators on correct pages, public signing page with mandatory consent disclosure and PDF preview showing signature locations, real-time signature drawing with mouse/touch support, automatic PDF processing using pdf-lib to overlay signatures on correct pages, SHA-256 hashing of original and signed documents, complete audit trails capturing signer identity/device/browser/OS/IP/timestamps, secure token-based public access (no authentication required), signed document storage in Replit Object Storage, and downloadable signed PDFs with verifiable hashes. Full database schema includes signature_requests, signature_request_recipients, signature_fields (with decimal coordinates and page numbers), signatures, signature_audit_logs, and signed_documents tables with proper indexing and relationships. Coordinates stored as percentages (0-100 with decimal precision) to ensure consistent rendering across different screen sizes and PDF dimensions. **Global Signature Requests View**: Accessible via "E-Signatures" link in TopNavigation menu at `/signature-requests`, provides staff with system-wide visibility of all signature requests across clients. Displays request status, client information, creator, and dates in consistent "MMM d, yyyy" format. API endpoints properly serialize all timestamp fields (including nested objects) to ISO strings for reliable frontend rendering. Both global view and client-specific views use consistent date formatting via date-fns. **Signature Request Builder (Nov 2025)**: Full-page wizard at `/clients/:clientId/signature-requests/new` for creating signature requests. 4-step workflow: (1) Select Document (upload new or choose existing PDF), (2) Place Fields (click-to-place signature/typed_name fields on PDF with drag-and-drop repositioning using @dnd-kit), (3) Add Recipients (select people who will sign), (4) Review & Send (summary and confirmation). Implements per-page dimension tracking via Map for accurate coordinate conversion, cross-page field movement via dropdown selector, unsaved changes warning with AlertDialog and beforeunload listener, duplicate field prevention (one signature + one typed_name per recipient), and DraggableField component with hover-visible controls (grip handle, page selector, remove button). Backend validates requests and rejects duplicates with 422 error. Known limitations: Field placement/drag-drop UI requires manual debugging - click handlers and field rendering need integration validation beyond automated testing.
