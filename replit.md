...
-   **Document E-Signature System**: UK eIDAS-compliant electronic signature feature enabling staff to send PDFs for signature via secure public links. Supports multiple signers per document with sequential or parallel workflows. Features include: interactive PDF canvas for signature field placement (signature and typed name fields), public signing page with mandatory consent disclosure, real-time signature drawing with mouse/touch support, automatic PDF processing using pdf-lib to overlay signatures, SHA-256 hashing of original and signed documents, complete audit trails capturing signer identity/device/browser/OS/IP/timestamps, secure token-based public access (no authentication required), signed document storage in Replit Object Storage, and downloadable signed PDFs with verifiable hashes. Full database schema includes signature_requests, signature_request_recipients, signature_fields, signatures, signature_audit_logs, and signed_documents tables with proper indexing and relationships.
