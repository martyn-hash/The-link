...
- **Dashboard Builder & Analytics**: Supports bar, pie, line charts, and number cards. Backend analytics endpoint (`/api/analytics`) aggregates data by various dimensions. Features interactive widget management, saved dashboards, and live filter integration that updates charts via React Query. Includes "Days Overdue" analysis and mini kanban board integration when a service filter is active.
