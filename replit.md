...
-   **Client Value Notifications**: When changing a project's stage, staff can send CLIENT-FACING notifications to client contacts (directors, shareholders, etc.). Features include: (1) Per-channel controls for email and SMS (SMS marked "Coming Soon"), (2) Recipient selection from client contacts via people table linked through clientPeople, with role badges, primary contact indicators, and opt-out status shown, (3) Tenant-wide Outlook integration - emails sent from staff's account using Microsoft Graph application permissions (requires admin-enabled `accessEmail` flag), with SendGrid fallback, (4) AI-assisted message drafting using voice recording for context-aware value messaging with automatic inclusion of completed work items from stage approval questionnaires and existing email template merge fields, (5) Rich text editing with TipTap for email body, (6) Sender status indicator showing admin-managed email access, (7) Staff Calendly link variable (`{staff_calendly_link}`) available for inclusion in notification templates - staff can add their Calendly booking link via Profile > Integrations. This replaces the legacy internal staff notification system with a client-focused communication channel.
...
-   **Resilient Project Scheduling Orchestrator**: The overnight project scheduling system (`server/scheduling-orchestrator.ts`) is designed to be robust against server restarts and outages. Features: (1) Startup catch-up that detects and executes missed scheduling runs when the server starts (scans up to 7 days back), (2) Database-based locking to prevent concurrent scheduling runs, (3) Idempotency protection that checks `scheduling_run_logs` before running to prevent duplicate project creation, (4) Automatic email notifications sent for catch-up runs with "[Catch-up Run]" prefix in summary, (5) Database query retry logic with exponential backoff for transient connection failures at both query and service-processing levels, (6) Pre-flight database health check before critical operations, (7) Automatic lock expiry for stuck runs (30 minutes), (8) Double-trigger prevention (30 second minimum between triggers), (9) Accurate failure status propagation (no false success reports), (10) Orchestrator-level retry (up to 2 retries with 30s delay) for transient failures like connection drops. The cron job runs at 01:00 UTC daily via the orchestrator's `executeScheduledRun()` function.

### Neon Database Configuration (Production Recommendations)
For optimal reliability of overnight scheduling jobs and other long-running database operations, configure your Neon project with these settings:
1. **Disable Auto-Pause**: In Neon Console > Project Settings, disable "Scale to zero" or set minimum compute to prevent cold starts during scheduled jobs
2. **Increase Idle Timeout**: Set idle session timeout to 300-600 seconds to prevent connections being dropped during long operations
3. **Use Pooled Connection Mode**: Enable connection pooling in Neon to handle multiple concurrent connections efficiently
4. **Connection String**: Use the pooled connection string (with `-pooler` suffix) for production workloads

The application's database client (`server/db.ts`) includes connection pooling settings optimized for Neon serverless, including automatic retry logic for transient connection failures.
