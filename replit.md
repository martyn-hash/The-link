...
-   **Stage Change Notifications**: When changing a project's stage, staff can optionally send notifications to stakeholders. Features include: (1) Per-channel controls allowing independent enable/disable of email, push, and SMS notifications, (2) Multi-recipient selection with checkboxes filtered by contact method (email for email channel, push subscription for push), (3) AI-assisted message drafting using voice recording that leverages stage approval questionnaire responses for context-aware value messaging, (4) Rich text editing with TipTap for email body composition, (5) SMS marked as "Coming Soon" pending VoodooSMS integration.
