...
-   **Communication & Collaboration**: Features push notification template management, an internal tasks system, standalone staff-to-staff messaging, email threading and deduplication via Microsoft Graph, and a multi-channel client notification and reminder system. **Internal Chat Refactoring (November 2025)**: Extracted InternalChatView component from /internal-chat page for reuse in both the standalone page and the /messages Internal Chat tab. The component accepts props (showNavigation, className, initialThreadId, onThreadSelected) and conditionally renders TopNavigation/BottomNav. Both implementations now share identical functionality including TiptapEditor, expand/collapse for long messages, PullToRefresh, voice notes, DOMPurify sanitization, and combined staff+project threading. The /messages page was cleaned to remove 400+ lines of duplicate Internal Chat logic, keeping only staffThreads query for unread badge counts. **Project Messaging (Enhanced November 2025)**: Rich text/HTML editing with Tiptap editor for message threads, supporting table creation and formatting with native table support, file attachments (up to 5 files, 25MB each) in message creation modal, and a large modal interface (max-w-6xl) with adequate editing space (300px editor height). The Tiptap editor provides a comprehensive toolbar with Bold, Italic, Underline, Strikethrough, text color picker (8 preset colors), Headings (H1-H3), bullet/numbered lists, Link insertion, advanced table features, and Undo/Redo. **Enhanced Table Support (November 2025)**: Google Docs-style 6×6 table insert picker with hover preview showing selected dimensions, 7 context-sensitive table editing buttons (add row before/after, delete row, add column before/after, delete column, delete table) that appear only when cursor is inside a table, and professional table styling with 1px solid #d0d7de cell borders, #f5f7fb header row background, 6px×8px cell padding, and #0F7195 selected cell outline. **Office Document Conversion (November 2025)**: Seamless preview of Microsoft Office documents (.docx, .xlsx) within the application - attachments convert to PDF for in-app preview using LibreOffice headless mode while downloads preserve the original format. Conversion features SHA-256-based caching in object storage at `.converted/{sha256}.pdf` for instant subsequent previews (first conversion 2-5 seconds, cached <1 second), authorization checks verifying thread access before conversion, and unified AttachmentPreview component handling images, PDFs, audio, and Office documents with loading states and error handling. Messages display as clean email-like cards with proper HTML rendering (bold, italic, underline, lists, tables) using prose formatting classes. Includes secure signed URL generation for attachment downloads and comprehensive email notifications via SendGrid to all thread participants. **Tiptap Migration (November 2025)**: Completed migration from ReactQuill to Tiptap across all 8 rich text editing locations: project messaging, email composition, email signatures, notification templates, email replies, and progress notes. All editors now use a shared TiptapEditor component with consistent toolbar and styling. **Message Expand/Collapse (November 2025)**: Intelligent content detection and expansion controls for improved readability in project messaging and internal chat. Messages automatically show expand/collapse buttons when content is tall-rendering (>500 characters, contains tables, images, or >5 paragraphs/list items). Collapsed messages use max-h-[200px] truncation with "Show more" (ChevronDown) toggle, expanded messages show full content with "Show less" (ChevronUp) toggle. State tracked independently per message using Set-based expandedMessages state, working seamlessly in both mobile and desktop layouts. **Internal Chat Enhancements (November 2025)**: Upgraded internal chat compose area with full TiptapEditor integration for rich text messaging. Replaced plain textarea with comprehensive rich text editing capabilities including formatting, tables, colors, and links. Integrated proper HTML content validation using hasMeaningfulContent() helper and DOMPurify sanitization with allowed tags (p, br, strong, em, tables, etc.). Plain text messages automatically escaped and wrapped in prose-formatted paragraphs with newline preservation. Both mobile and desktop layouts support expand/collapse functionality for long messages. **Email Configuration (November 2025)**: Updated SendGrid email sender configuration across all services (sendgridClient.ts, sendgridService.ts, sendgrid.ts) to use professional sender display name "The Link" instead of generic "link". Sender objects now use format {email: fromEmail, name: "The Link"} for improved brand recognition in client communications. **UI Component Fixes (November 2025)**: Fixed color picker z-index issue in kanban modal by elevating Popover component to z-[100], ensuring color picker appears above Dialog content (z-[70]) and preventing UI layering conflicts in nested modal contexts.
